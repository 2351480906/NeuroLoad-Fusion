...
class BIOTEncoder(nn.Module):
    ...
    def forward_features(self, x, n_channel_offset=0):
        """
        提取特征的方法，逻辑与 forward 核心部分一致。
        x: [batch_size, channel, ts]
        output: [batch_size, emb_size]
        """
        emb_seq = []
        ...
        for i in range(x.shape[1]):
            ...
            # 2. Patch Embedding (频率维度投影)
            channel_spec_emb = self.patch_embedding(channel_spec_emb)
            batch_size, ts, _ = channel_spec_emb.shape
            
            # 3. 添加 Channel Token (区分不同脑电通道)
            channel_token_emb = (
                self.channel_tokens(self.index[i + n_channel_offset])
                .unsqueeze(0)
                .unsqueeze(0)
            ...
            # 注意：这里去掉了 perturb 逻辑，保证特征提取的确定性
            emb_seq.append(channel_emb)
        ...
        # 7. 全局平均池化 (Global Average Pooling)
        # 将序列特征压缩为单个向量: (batch_size, emb_size)
        emb = emb.mean(dim=1)
        
        return emb

    def forward_tokens(self, x, n_channel_offset=0):
        """
        Return the EEG spectral token sequence before global pooling.
        x: [batch_size, channel, ts]
        output: [batch_size, n_channels * n_stft_frames, emb_size]
        """
        emb_seq = []
        for i in range(x.shape[1]):
            channel_spec_emb = self.stft(x[:, i : i + 1, :])
